...
export const pagesize = T.get('int').swg({
  in: 'query',
  name: 'pagesize',
  description: '分页大小',
  req: false,
...
export const page = T.get('int').swg({
  in: 'query',
  name: 'page',
  description: '分页页数(不能同时使用page和next)',
})

export const next = T.get('bigint').swg({
  in: 'query',
  name: 'next',
...
const filter = T.get('object').swg({
  description: '筛选参数，{ key, symbol, value }',
  properties: {
    key: T.get('string').swg(),
    symbol: T.get('string').swg(),
    value: {
      type: ['string', 'number'],
    },
  },
...
const filters = T.get('array').swg({
  description: '筛选参数数组',
  items: filter,
})

const orderByItem = T.get('object').swg({
  description: '排序参数, { by, sort, ... }',
  properties: {
    by: T.get('string').swg(),
    sort: T.get('string').swg(),
  },
})


const orderBy = T.get('array').swg({
  description: '排序参数数组',
  items: orderByItem,
})

export const pagingData = T.get('object').swg({
...
export const total = T.get('int').swg({
  description: '列表总条目',
})

export const eol = T.get('boolean').swg({
